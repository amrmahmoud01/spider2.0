...
class ComfowearSpider(scrapy.Spider):
    ...
    def parse_product(self,response, gender):
        if response.css(".price-item.price-item--sale.price-item--last ::text").get():
            salePrice = re.sub(r"[^\d.]","",response.css(".price-item.price-item--sale.price-item--last ::text").get().strip())
        else: 
            salePrice=0
